...
import { EMPLOYER_JOBS, APPLICANTS, type Applicant, type ApplicationStage } from "@/features/employer/api/employer.api";
...
export default function ApplicationsKanbanPage() {
  ...
  const [board, setBoard] = useState<Applicant[]>(APPLICANTS.filter((a) => a.stage !== "Rejected"));
  const [dragId, setDragId] = useState<string | null>(null);
  const [overStage, setOverStage] = useState<ApplicationStage | null>(null);

  const visible = useMemo(
    () => (jobFilter === "all" ? board : board.filter((a) => a.jobId === jobFilter)),
    [board, jobFilter],
  );

  const byStage = (stage: ApplicationStage) => visible.filter((a) => a.stage === stage);

  const drop = (stage: ApplicationStage) => {
    if (dragId) setBoard((prev) => prev.map((a) => (a.id === dragId ? { ...a, stage } : a)));
    setDragId(null);
    setOverStage(null);
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          ...
          {EMPLOYER_JOBS.map((j) => <option key={j.id} value={j.id}>{j.title}</option>)}
        </select>
      </div>
                ...
                {cards.map((a) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-sm font-extrabold shrink-0 text-primary-600">{a.match}%</span>
                    </div>
                  </div>
                ))}
                {cards.length === 0 && (
